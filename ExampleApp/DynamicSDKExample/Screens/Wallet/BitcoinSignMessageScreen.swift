...
struct BitcoinSignMessageScreen: View {
  ...
  var body: some View {
    ScrollView {
      VStack(alignment: .leading, spacing: 20) {
        InfoCard(
          ...
          content: "Sign a message using the Bitcoin wallet.",
          copyable: false
        )
    ...
    .navigationTitle("Bitcoin Sign Message")
  ...
  private func signMessage() {
    ...
    Task {
      do {
        let result = try await DynamicSDK.instance().bitcoin.signMessage(
          walletId: walletId,
